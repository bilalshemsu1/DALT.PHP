...
test('the Fullstack manifest describes all planned parts and only real Fullstack lessons', function () {
    ...
    expect(array_map(static fn (string|int $part): int => (int) $part, array_keys($track['parts'])))->toBe(range(0, 12))
        ...
        ->and($track['parts']['01']['lessons'])->toBe(['22-fs01-1-data-functions-transformations', '56-fs01-2-functions-arrays-and-transformations', '23-fs01-2-modules-async-and-failure'])
        ->and($track['parts']['01']['milestones'][0])->toMatchArray([
            'id' => 'B01',
            'route' => '/learn/fullstack/build/b01',
            'prerequisites' => ['22-fs01-1-data-functions-transformations', '56-fs01-2-functions-arrays-and-transformations', '23-fs01-2-modules-async-and-failure'],
        ])
        ...
        ->and($fullstackLessons[10]['prerequisites'])->toBe(['26-fs02-3-unions-narrowing-and-unknown'])
        ->and($fullstackLessons[11]['prerequisites'])->toBe(['27-fs02-4-functions-generics-and-reusable-types'])
        ->and($fullstackLessons[12]['prerequisites'])->toBe(['28-fs02-5-runtime-boundaries'])
        ->and($fullstackLessons[13]['prerequisites'])->toBe(['29-fs03-1-components-jsx-and-typed-props'])
        ->and($fullstackLessons[14]['prerequisites'])->toBe(['30-fs03-2-state-and-events'])
        ->and($fullstackLessons[15]['prerequisites'])->toBe(['31-fs03-3-forms-and-state-design'])
        ->and($fullstackLessons[16]['prerequisites'])->toBe(['32-fs03-4-tailwind-and-accessible-ui'])
        ->and($fullstackLessons[17]['prerequisites'])->toBe(['33-fs04-1-fetching-data-and-effects'])
        ->and($fullstackLessons[18]['prerequisites'])->toBe(['34-fs04-2-mutating-server-data'])
        ->and($fullstackLessons[19]['prerequisites'])->toBe(['35-fs04-3-separating-transport-from-ui'])
        ->and($fullstackLessons[20]['prerequisites'])->toBe(['36-fs05-1-designing-the-application-api'])
        ->and($fullstackLessons[21]['prerequisites'])->toBe(['37-fs05-2-relational-modeling-and-migrations'])
        ->and($fullstackLessons[22]['prerequisites'])->toBe(['38-fs05-3-crud-queries-and-transaction-boundaries'])
        ->and($fullstackLessons[23]['prerequisites'])->toBe(['39-fs06-1-backend-api-behavior-tests'])
        ->and($fullstackLessons[24]['prerequisites'])->toBe(['40-fs06-2-users-passwords-sessions-and-csrf'])
        ->and($fullstackLessons[25]['prerequisites'])->toBe(['41-fs06-3-authorization-and-ownership'])
        ->and($fullstackLessons[26]['prerequisites'])->toBe(['42-fs07-1-urls-and-react-router'])
        ->and($fullstackLessons[27]['prerequisites'])->toBe(['43-fs07-2-authentication-in-the-frontend'])
        ->and($fullstackLessons[28]['prerequisites'])->toBe(['44-fs07-3-test-frontend-behavior'])
        ->and($fullstackLessons[29]['prerequisites'])->toBe(['45-fs08-1-client-state-versus-server-state'])
        ->and($fullstackLessons[30]['prerequisites'])->toBe(['46-fs08-2-mutations-invalidation-and-optimistic-ui'])
        ->and($fullstackLessons[31]['prerequisites'])->toBe(['47-fs08-3-context-reducers-and-zustand'])
        ->and($fullstackLessons[32]['prerequisites'])->toBe(['48-fs09-1-custom-hooks-and-feature-boundaries'])
        ->and($fullstackLessons[33]['prerequisites'])->toBe(['49-fs09-2-build-pipeline-configuration-and-failure-boundaries'])
        ->and($fullstackLessons[34]['prerequisites'])->toBe(['50-fs10-1-containers-around-the-application'])
        ->and($fullstackLessons[35]['prerequisites'])->toBe(['51-fs10-2-builds-health-and-debugging'])
        ->and($fullstackLessons[36]['prerequisites'])->toBe(['52-fs11-1-query-performance-and-postgresql-capabilities']);
});

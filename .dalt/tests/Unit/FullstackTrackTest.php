...
test('the Fullstack manifest describes all planned parts and only real Fullstack lessons', function () {
    ...
    expect(array_map(static fn (string|int $part): int => (int) $part, array_keys($track['parts'])))->toBe(range(0, 12))
        ...
        ->and($track['parts']['11']['lessons'])->toBe(['52-fs11-1-query-performance-and-postgresql-capabilities', '76-fs11-2-explain-composite-and-partial-indexes', '53-fs11-2-transactions-concurrency-and-row-isolation'])
        ->and($track['parts']['11']['milestones'][0])->toMatchArray(['id' => 'B11', 'route' => '/learn/fullstack/build/b11', 'prerequisites' => ['52-fs11-1-query-performance-and-postgresql-capabilities', '76-fs11-2-explain-composite-and-partial-indexes', '53-fs11-2-transactions-concurrency-and-row-isolation']])

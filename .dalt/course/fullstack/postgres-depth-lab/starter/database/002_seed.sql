...
SELECT
    1 + (n % 2),
    1 + (n % 10),
    -- Different moduli, so the words combine in varied ways instead of always
    -- appearing as the same fixed sentence.
    'Issue ' || n || ' ' || (ARRAY['timeout', 'deadlock', 'regression', 'crash', 'typo'])[1 + (n % 5)],
    'The ' || (ARRAY['login form', 'issue list', 'search page', 'export job', 'webhook'])[1 + (n % 7) % 5]
        || ' reported a ' || (ARRAY['timeout', 'deadlock', 'regression', 'crash', 'typo'])[1 + (n % 5)]
        || ' after the ' || (ARRAY['deploy', 'migration', 'rollback', 'restart', 'upgrade'])[1 + (n % 3)] || '.',
